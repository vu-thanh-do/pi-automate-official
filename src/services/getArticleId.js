...
async function getArticleId() {
    ...
    try {
      const url = "https://pivoice.app/vjson/home/list/index/0_1?time=03190420";
      ...
      if (!listResponse.data || !listResponse.data.data || !listResponse.data.data.home_1) {
        console.log(`❌ Không tìm thấy dữ liệu bài viết. Response data: ${JSON.stringify(listResponse.data)}`);
      ...
      if (!article.id) {
        console.log(`❌ Bài viết không có ID. Bài viết: ${JSON.stringify(article)}`);
